...
- **Mouse and touch first.** Nothing requires a keyboard shortcut (D3). Shortcuts (Cmd/Ctrl+K, Escape, q or w to close the focused window, a to tile every window and again to put them back) are extras, and never fire while a field, dialog or menu has the keys.
- **Phones are first class.** Under about 768px (or short and touch, as when held sideways) each window is full screen, one at a time, with a switcher (D17). Touch targets are 44px. The page scrolls rather than a pane, as mobile browsers only draw content under their toolbars for a scrolling document; keep sticky elements to the panel, a window's tab and toolbar, and the dock, as iOS Safari fills a flat bar under its toolbar once anything else sticks mid-page.
- **Small.** The core shell stays at or under 40KB gzipped and every on-demand bundle at or under 12KB, enforced by `make test`. Features outside the core load on first use (`loader.js`).
...
- **Folders.** Pages with `layout: folder`. They list child pages, `include`d pages and data-file items, so collections such as Favourites and Projects need content and data only, no code.
...
- **Palettes** (`[data-palette]`, in `lazy/control-panel.css`) replace tokens only: Haiku, BeOS, Xfce and Sage, plus the lighter Snow, Mint and Peach with near-white frames and panel, and Synthwave (pink tabs, lavender or indigo frames, cyan links in dark).
