...
Appearance is six independent choices (window style, palette, mode, dock, wallpaper, CRT effect) plus themes that set all but the mode at once (D39). Every option and theme is data in `assets/js/deskbar/lib/appearance.js`, where themes are `PRESETS` since `theme` already names the mode; the Control panel only draws it. A theme is shown as picked whenever the current choices add up to it, so nothing about themes is stored.

- **Palettes** (`[data-palette]`, one stylesheet each in `palettes/`) replace tokens only: Haiku and the cleaner Crisp, BeOS, Xfce and Sage, the lighter Snow, Mint and Peach and the pastel Lilac, Blossom and Lemon with near-white frames, Synthwave (pink tabs, lavender or indigo frames, cyan links in dark), four borrowed from editor themes and places: Rosé (Rosé Pine), Ember (Gruvbox), Solar (Solarized) and Lagoon (mid-century poolside), and the bolder Cobalt and Racing Green. Their token blocks sit at `:where(:root)` specificity, so a look or glass style outranks them whatever order the sheets load in.
...
- **Whole looks** (D38): Platinum (Mac OS 9), Clearlooks (GNOME 2), Phosphor (an amber CRT), Broadsheet (neo-brutalist newsprint), Synthwave (80s outrun: neon-edged indigo windows, a striped sun over a perspective grid, dark only), three Synthwave variants in the same colours over flat patterned wallpapers (Vector: angular cyan-lined windows over a triangle lattice; Memphis: hard offset shadows over 80s confetti; Night Drive: rounded windows with badged indigo tabs in chroma-split capitals over flat stripe bands) and Pixel (retro pixel art: ink-ringed windows with a dithered sun title bar, Pixelify Sans chrome, a dithered sky over tiling hills, day in light and a starry dusk in dark, with colour variants, docks and wallpapers of its own). Each is one stylesheet in `assets/css/deskbar/looks/` that draws a window style, a wallpaper, a dock and their Control panel thumbnails, all named after the look; its variants are `<look>-<variant>` values in the same stylesheet. Its wallpaper and dock go with any window style, so they carry literal colours rather than the look's tokens. Its window style sets its own colours, so `LOOKS` in `lib/appearance.js` turns Colours off while it is on (and Mode, for a dark-only look), or offers the look's own colour variants in their place. A look with a different image per theme sets a custom property under `[data-theme=dark]` and under `[data-theme=auto]` with `prefers-color-scheme`, since `light-dark()` takes colours only.
  - Scope every rule to `[data-deco=<name>]`, `.wm[data-wall=<name>] body`, `.wm[data-dock=<name>] #dock` or `.cp-deco.<name>` / `.cp-wp.<name>` / `.cp-dk.<name>`. Nothing may apply when that part is off. The Control panel preloads every look's stylesheet for its thumbnails.
  - A look must cover the whole shell, not just windows: panel, menu, context menu, dock, switcher, Spotlight, toolbars, inputs, Tracker, the reader and phone mode. The Liquid Ass and Clear blocks in `lazy/control-panel.css` are the checklist. A look that clips `.views` (`overflow: hidden`, for rounded corners) must undo it in phone mode, or the reader toolbar stops sticking under the title bar.
...
- **CRT effects** (`[data-crt]`, `effects/crt.css`): full-screen overlays on `html::before` and `html::after` that never take the pointer and never filter or transform the page, so fixed windows stay put. Those two pseudo-elements are theirs, so a look draws elsewhere (Platinum's screen corners are in the panel). Phosphor's tube is one of the effects, so it goes over any look. Popovers and dialogs sit in the top layer above them, so the monochrome tints miss menus.
...
- Building a new aesthetic as separate palettes, window styles and wallpapers (D38). A look is designed as a set, and a theme (D39) keeps it one choice away.
