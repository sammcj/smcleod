...
- Visitors change the desktop in the Control panel, a page with `window: control-panel` (the example site has `content/control-panel.md`, aliased from `/appearance/`). Each pane has an address (`?pane=posts`, `?pane=system`, none for the first), which layout links keep:
  - **Appearance:** palette (Haiku, BeOS, Xfce, Sage, the light Snow, Mint and Peach, the neon Synthwave, Rosé, Ember, Solar and Lagoon), light (the default), dark or auto (follows the OS), window style (Haiku, BeOS, Flat, Clear, Liquid Ass, Platinum, Clearlooks, Phosphor, Broadsheet, Synthwave and its variants Vector, Memphis and Night Drive, Pixel), wallpaper and dock style (Glass, Deskbar, Panel). Liquid Ass spoofs Apple's Liquid Glass: over-blurred glass, mismatched corner radii, traffic lights on the left and wobbly buttons; Clear is the same glass without the joke. Platinum (Mac OS 9), Clearlooks (GNOME 2), Phosphor (an amber CRT), Broadsheet (neo-brutalist newsprint), Synthwave (an 80s outrun sunset over a neon grid) and Pixel (modern pixel art in Pixelify Sans from `static/fonts/pixelify-sans/`) are whole looks that also set their own colours and dock skin, so the Colours and Dock choices are disabled while one is on (Phosphor and Synthwave also fix Mode to dark). Picking any of these styles also picks its wallpaper and dock, and leaving it puts back the ones it replaced (`deskbar:lookWas`) unless the visitor changed them meanwhile. A new look is `assets/css/deskbar/looks/<name>.css`, with every rule scoped to `[data-deco=<name>]`, `[data-wall=<name>]` or its `.cp-deco.<name>` and `.cp-wp.<name>` thumbnails, plus an entry in `LOOKS` and the option lists in `lazy/control-panel.js`. It builds as its own stylesheet, which head.html links before first paint when it is the stored style or wallpaper
